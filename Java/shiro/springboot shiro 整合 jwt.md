...
```java
import com.simple.api.config.JwtToken;
...
public class JwtUtil {
    ...
    public static void main(String[] args) {
        ...
        System.out.println(verify(sign,"18888888888", "123456"));;
    }
}
```
...
#### 4.实现shiro的AuthenticationToken接口的类JwtToken
...
public class MyRealm extends AuthorizingRealm {
    ...
    protected AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken auth) throws AuthenticationException {
        ...
        SysUser userBean = sysUserService.getUserInfoByName(username);
        if (userBean == null) {
            log.error("用户不存在!)");
...
```java
import com.simple.api.filter.JwtFilter;
